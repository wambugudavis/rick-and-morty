        ...
        <div className="grid grid-cols-5 gap-6 py-6">
            {
                residents
                    .filter((character) => {
                        const term = searchParams.get('resident')?.toString().toLowerCase()
                        return term === undefined ? character : character.name.toLowerCase().includes(term || '');
                    })
                    .map((character) => {
                        return (
                            <Link key={character.id} href={'/character/' + character.id + '/episodes'}
